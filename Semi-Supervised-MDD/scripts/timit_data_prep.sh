...
conf_dir=`pwd`/Semi-Supervised-MDD/config
prepare_dir=`pwd`/data
map_file=$conf_dir/phones.60-48-40.map
phoneme_map=$2
...
for x in train_timit; do
  ...
  for y in wrd phn; do
    ...
    if [ $y == phn ]; then 
        ...
        python Semi-Supervised-MDD/scripts/normalize_phone.py --map $map_file --to $phoneme_map --src $prepare_dir/$x/${y}_text.tmp --tgt $prepare_dir/$x/${y}_text
